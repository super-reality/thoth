...
export class discord_client {
  googleSpeechClient = new googleSpeech.SpeechClient()
            ...
            async (channel: {
              ...
              if (
                ...
                receiver.speaking.on('start', userId => {
                  ...
                  const converted = new ConvertTo1ChannelStream()
                  audioStream.pipe(converted).pipe(recognizeStream)
                })

                connection.on('speaking', (user, speaking) => {
                  console.log('speaking called:', speaking)
                  if (!speaking) {
                    return
                  }
                  ...
                  //send stream to google speech api
                })
                return false
              }
            }
  ...
  async playAudio(connection, filePath) {
